...
def inject_patches(original: Path, manifest_bin: bytes) -> None:
    UNSIGNED.parent.mkdir(exist_ok=True)
    patched_assets = {
        "assets/ShareSDK.xml": (APKTOOL_SRC / "assets" / "ShareSDK.xml").read_bytes(),
    }
    with zipfile.ZipFile(original, "r") as zin, zipfile.ZipFile(UNSIGNED, "w", allowZip64=True) as zout:
        for info in zin.infolist():
            ...
            elif name in patched_assets:
                zout.writestr(copy_info(info), patched_assets[name])
...
def main() -> None:
    ...
    inject_patches(original, manifest_bin)
    sign(original)
